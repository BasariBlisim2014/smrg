...
class WC_Meta_Box_Product_Data {
	...
	public static function output( $post ) {
		...
		<div class="panel-wrap product_data">
			...
			<div id="general_product_data" class="panel woocommerce_options_panel"><?php
				...
				echo '<div class="options_group hide_if_grouped">';
					...
					woocommerce_wp_checkbox( array( 'id' => '_is_fancy', 'label' => __( 'Is Editable', 'woocommerce' ),  'description' => __( 'If checked then user can edit this product in frame editor tool', 'woocommerce' ) ) );
					echo "<div id='extra_params'>";
					woocommerce_wp_text_input( array( 'id' => '_img_width', 'data_type' => 'price', 'label' => __( 'Image Width', 'woocommerce' ) , 'description' => 'Add original width of image' ) );
					
					woocommerce_wp_text_input( array( 'id' => '_img_height', 'data_type' => 'price', 'label' => __( 'Image Height', 'woocommerce' ), 'description' => 'Add original height of image' ) );
				
					do_action('woocommerce_product_options_is_fancy');
					...
					foreach($category_terms as $category_term) {
						$option_arr[$category_term->slug]=$category_term->name;
					}	
				
					woocommerce_wp_select( array( 'id' => '_fpd_cat', 'label' => __( 'Fancy Category', 'woocommerce' ), 'description' => sprintf( __( 'Choose a Category from Dropdown', 'woocommerce' ), 'http://schema.org/' ), 'options' =>$option_arr) );
					echo "</div>";
		...
		woocommerce_wp_text_input( array( 'id' => '_delivery_days', 'data_type' => 'number', 'label' => __( 'Delivered in Days', 'woocommerce' ) , 'description' => 'Add The days within that the product will be delivered' ) );
				...
				echo '<div class="options_group pricing show_if_simple show_if_external">';
					...
					woocommerce_wp_text_input( array( 'id' => '_regular_price_euro', 'label' => __( 'Regular Price', 'woocommerce' ) . ' (&euro;)', 'data_type' => 'price' ) );
	...
	public static function save( $post_id, $post ) {
		...
		/****************************** code for saving editable product **********************************/
		if(isset($_POST['_delivery_days']) && $_POST['_delivery_days']!='')
		{
			 update_post_meta( $post_id, '_delivery_days', stripslashes( $_POST['_delivery_days'] ) );
		}
		if(isset($_POST['_regular_price_euro']) && $_POST['_regular_price_euro']!='')
		{
			update_post_meta( $post_id, '_regular_price_euro', stripslashes( $_POST['_regular_price_euro'] ) );
		}
		if ( isset( $_POST['_is_fancy'] ) ){
		
			
			 update_post_meta( $post_id, '_is_fancy', stripslashes( 'yes' ) );
			  update_post_meta( $post_id, '_img_width', stripslashes( $_POST['_img_width'] ) );
			 update_post_meta( $post_id, '_img_height', stripslashes( $_POST['_img_height'] ) );
			 
			 	$img_id=get_post_thumbnail_id($post_id);
				$source  = wp_get_attachment_url($img_id);
				wp_delete_object_term_relationships($img_id, 'fpd_design_category');
				$attachment = array(
					'ID'           => $img_id,
					'menu_order' => 0
				);
				...
				wp_set_object_terms( $img_id, $_POST['_fpd_cat'], 'fpd_design_category', true );
				
				$parameters='enabled=1&title='.$_POST['post_title'].'&description=&size=&colors=&price='.$_POST['_regular_price'].'&img_width='.$_POST['_img_width'].'&img_height='.$_POST['_img_height'];
				
				update_post_meta( $img_id, 'fpd_parameters', $parameters);
				update_post_meta( $post_id, '_fpd_cat', stripslashes( $_POST['_fpd_cat'] ) );
				
		}else
		{
			 update_post_meta( $post_id, '_is_fancy', stripslashes( 'no' ) );
		}
